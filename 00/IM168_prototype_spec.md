...
| Ball colors `c1`–`c10` | `#E0322B #F5821F #3FA9F5 #27AE60 #9B59B6 #16A085 #E67E22 #34495E #C0392B #2C3E50` | Per-position lottery ball background colors (10-ball PK10 result display) |
...
- **Top row:** back chevron (‹) → returns to channel; game switcher title (game name + small "▾" caret + room name subtitle, e.g. **"PK10 ▾ 幸运组"**) → opens game lobby; spacer; **balance pill** (wallet icon + "¥400") → opens wallet; **"⋮" more menu** → opens More screen.
...
1. **Playbar** — horizontally scrollable market-category pills specific to the active game:
   - PK10: **两面** (Two-Sided) / **1-10名** (Position 1–10) / **冠亚和** (Champion+Runner-up Sum), plus a **紧凑/宽松** (Compact/Loose) view-mode segmented toggle, plus a "🧾 我的注单" (my bets) shortcut icon.
...
**Layout:**
- Subhead: back · "全部游戏" · 🔍
- Section "已开放" (Available): 2-column tile grid (`.lobgrid`) — **极速赛车 PK10** ("已开放" tag, 🏎 icon, "每 80 秒一期" every-80-seconds subtitle) and **时时彩 SSC** ("已开放" tag, 🎰 icon, "每 5 分钟一期").
