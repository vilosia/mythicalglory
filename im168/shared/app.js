...
(function(){
  ...
  function colorTxNegatives(scope){
    if(!scope||!scope.querySelectorAll)return;
    scope.querySelectorAll('.txlist .bi .r b').forEach(function(b){
      if(b.classList.contains('win'))return;
      if(/[\u2212-]/.test(b.textContent))b.classList.add('lose');
    });
  }
...
(function(){
  ...
  function renderHistory(rows,streaks){
    ...
    var history=(rows||[]).map(function(row){
      return '<div class="histrow"><span>'+row.period+'</span><span class="histballs">'+ballImgHtml(row.balls)+'</span></div>';
    }).join('');
  ...
  function setGame(g,el){
    ...
    root.querySelectorAll('[data-game-option]').forEach(function(option){
      var selected=option.getAttribute('data-game-option')===g;
  ...
  function renderDash(){
    ...
    var h='';
    if(!dashPlans.length){
      h+='<div class="ehero"><img class="eart" src="shared/plan-hero.png" alt="" onerror="this.remove()"><h4>「自动投」让策略替你盯盘</h4><p><b>追长龙</b>不用手点，定好规则后逐期自动下注，</p><p><b>实时追踪，随时急停</b>。</p></div>';
  ...
  document.addEventListener('click',function(e){
    ...
    if(act){
      ...
      else if(a==='autointro'){var dwi=root.querySelector('#drawer');if(dwi){dwi.classList.remove('open');}openAutoGuide(true);}
      ...
      else if(a==='gameheadpick'){setGame(arg,null);closeGameHeadMenu();}
...
(function(){
  ...
  /* 完整的本轮监测账本：[时间, 期号, 状态, 已结算盈亏, 实际投入]。计划 15 期，当前已开奖 10 期；未触发期也保留供逐期核对。 */
  ...
  function openSharedRun(idx){
    ...
    var f=shareFacts(entry),state=shareState(entry),sm=shareStatusMeta(state,entry);shareViewIdx=idx;sheetMode='sharelive';cur=null;editIdx=-1;
    ...
    b.innerHTML='<div class="lss-detail-v2"><div class="lss-detail-scroll"><div class="lss-detail-summary">'
      ...
      +'</div><div class="cf-btns lss-detail-actions"><button class="ghost" data-act="stgclose">关闭</button><button class="cta" data-act="stgshareapply" data-arg="'+idx+'">'+shareUseLabel(state)+'</button></div></div>';
  ...
  (function(){
    ...
    autoBar.innerHTML='<span class="demo-t">DEMO · 自动投场景</span>'+demoButtons(DEMO_AUTOS,'ademo')+'<span class="demo-section">规则检查</span><button data-act="specgallery" aria-pressed="false">15 · 执行记录 · 规则总览</button>';
  ...
  function estBoxHtml(){
    ...
    var h='<div class="est-row"><span class="stg-est">'+et+'</span>'
      +(isMulti?('<button type="button" class="est-toggle'+(estOpen?' open':'')+'" data-act="stgestx" aria-expanded="'+(estOpen?'true':'false')+'"><span>'+(estOpen?'收起':'每回合投入')+'</span><svg viewBox="0 0 16 16" aria-hidden="true"><path d="m6 3 5 5-5 5"/></svg></button>'):'')+'</div>';
    if(step>=2){
  ...
  document.addEventListener('click',function(e){
    ...
    else if(a==='stgstartgo'){var spg=stgPlans[parseInt(arg,10)];if(spg){spg.status='run';spg.pnl=spg.pnl||0;startRun(spg);spg.reports=spg.reports||[];spg.reports.unshift({t:'09:02',title:'策略已启用 · 运行中',s:'开始时间 09:02；从下一期开始按当前设置监测。',set:true,eventType:'enabled',origin:true});cur=null;closeSheet();toast('策略已启用 · 正在运行');renderHome();}}
