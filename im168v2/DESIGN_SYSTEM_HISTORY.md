# 2026-07-25 · v2.1 字号底线与颜色归一（全站实施记录）

本轮依据《IM168 原型 UI/UX 设计规范审计》执行，改动已落地 `shared/design-tokens.css`、`shared/styles.css`、`shared/app.js`：

1. **字号底线 12px**：清除全部 9/10/11px 字号（382 处 CSS 声明 + 22 处 font 简写），统一升至 12px；12.5px 归一为 13px。唯一例外：数字角标 `--fs-badge: 11px`（角标容器 16→18px 等比放大）。`--fs-micro` 层级删除；`--feed-fine/--feed-meta` 私有变量并入 12px。
2. **核心阅读场景升级**：聊天气泡正文 12→15px；投注选项名 14→15px；开奖球号 12px（球体 19→21px）；迷你球 16→18px、开奖详情球 18→20px。
3. **对比度修复**：`--muted` 由 #9CA3AF（白底 2.5:1，不达标）加深为 #6B7280（4.8:1）；新增 `--faint: #98A2B3` 仅限装饰。
4. **颜色归一**：约 1,400 处裸 hex 替换为 token 引用；外壳蓝 #1F6FFF 并入品牌蓝 `--blue`；13 个近似浅灰并入 `--fill`、11 个近似线灰并入 `--line`/`--line-strong`、9 个近似浅蓝并入 `--blue-soft`/`--blue-line`；英雄卡渐变 token 化为 `--hero-grad`。不同 hex 由 393 收敛至约 310（剩余为球色、图表、演示层等长尾，后续按模块处理）。
5. **语义修正**：`colorTxNegatives` 只对游戏盈亏类亏损标红，普通上分/下分支出恢复主文字色（执行 C11）。
6. **字重收敛**：规范改为 400/500/700 三档，与实际加载字重一致，移除 600。

备份：`_ds-migration-backup/styles.css.bak-20260725-typo-color`、`_ds-migration-backup/design-tokens.css.bak-20260725`。
